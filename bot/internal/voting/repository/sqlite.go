...
// SQLiteRepository реализует Repository с использованием SQLite.
type SQLiteRepository struct {
	db *sql.DB
}

// NewSQLiteRepository создает новый SQLite репозиторий.
func NewSQLiteRepository(db *sql.DB) *SQLiteRepository {
	return &SQLiteRepository{db: db}
}

// CreateSession создает новую сессию голосования.
func (r *SQLiteRepository) CreateSession(chatID int64, messageID int, photoFileID string, facesCount int, duration time.Duration) (*voting.Session, error) {
...
func (r *SQLiteRepository) GetSession(sessionID string) (*voting.Session, bool) {
	ctx := context.Background()
...
func (r *SQLiteRepository) GetSessionByMessage(chatID int64, messageID int) (*voting.Session, bool) {
	ctx := context.Background()
...
func (r *SQLiteRepository) CanCreateSession(chatID int64, minInterval time.Duration) bool {
	ctx := context.Background()
...
func (r *SQLiteRepository) DeleteSession(sessionID string) {
	ctx := context.Background()
	r.db.ExecContext(ctx, `DELETE FROM sessions WHERE id = ?`, sessionID)
}

// CleanupExpired удаляет истекшие сессии старше указанного времени.
func (r *SQLiteRepository) CleanupExpired(olderThan time.Duration) {
	ctx := context.Background()
	cutoff := time.Now().Add(-olderThan)
	r.db.ExecContext(ctx, `
		DELETE FROM sessions
		WHERE is_active = 0 AND end_time < ?
	`, cutoff)
}

// Vote регистрирует голос в сессии.
func (r *SQLiteRepository) Vote(sessionID string, userID int64, faceIndex int) error {
	ctx := context.Background()
	...
	if err != nil {
		...
		if err.Error() == "UNIQUE constraint failed: votes.session_id, votes.user_id" {
...
func (r *SQLiteRepository) GetSessions(ctx context.Context, limit, offset int, chatID *int64, isActive *bool) ([]*voting.Session, error) {
	...
	for rows.Next() {
		...
		// Загружаем голоса
		voteRows, err := r.db.QueryContext(ctx, `SELECT user_id, face_index FROM votes WHERE session_id = ?`, s.ID)
		if err != nil {
...
func (r *SQLiteRepository) GetSessionsCount(ctx context.Context, chatID *int64, isActive *bool) (int, error) {
...
func (r *SQLiteRepository) GetChats(ctx context.Context, limit, offset int) ([]ChatInfo, error) {
	rows, err := r.db.QueryContext(ctx, `
...
func (r *SQLiteRepository) GetChatInfo(ctx context.Context, chatID int64) (*ChatInfo, error) {
	var c ChatInfo
	var lastActive sql.NullTime
...
func (r *SQLiteRepository) GetStats(ctx context.Context) (*Stats, error) {
	...
	// Активные сессии
	err = r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM sessions WHERE is_active = 1`).Scan(&stats.ActiveSessions)
	if err != nil {
